...
func VersionOrdinal(version string) string {
	...
	for i := 0; i < len(version); i++ {
		...
		if vo[j]+1 > maxByte {
			fmt.Errorf("invalid version : %s", version)
		}
		vo = append(vo, b)
		vo[j]++
	}
...
func (iosVersion *IOSVersion) LessThan(target string) bool {
	v1, v2 := VersionOrdinal(iosVersion.version), VersionOrdinal(target)
	if v1 < v2 {
		return true
	} else {
		return false
...
func (iosVersion *IOSVersion) GreaterThan(target string) bool {
	v1, v2 := VersionOrdinal(iosVersion.version), VersionOrdinal(target)
	if v1 > v2 {
		return true
	} else {
